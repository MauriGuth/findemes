...
- **`railway.json` at the repository root** (Railway reads it from the service root, which must stay `/` for the Docker context): `builder: DOCKERFILE`, `dockerfilePath: apps/api/Dockerfile`, `preDeployCommand: node_modules/.bin/prisma migrate deploy && node dist/seed.js` _(seed added in phase 1: idempotent catalog upsert)_, `healthcheckPath: /health`, `watchPatterns` limited to the API and shared packages.
...
- Because `railway.json` is config-as-code, the dashboard must not override the build or start command: the runtime image has no pnpm, so a custom `pnpm --filter … start` fails. Phase 1 also removed BuildKit cache mounts from the Dockerfile: Railway's Dockerfile validation rejected `--mount=type=cache`.

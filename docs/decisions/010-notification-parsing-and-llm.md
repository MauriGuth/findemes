...
- **Cost control.** `LLM_DAILY_CAP` calls per user per Argentine day (default 30); every call writes `LlmUsage` (model, tokens, cost in micro-dollars, outcome) and never the text. Failed calls (`outcome = error`) are logged but do not count toward the cap, and a FAILED event is retried only after a 5-minute backoff, so an Anthropic outage plus the phone's resends cannot burn the day's cap and leave real payments unrecognized. Over the cap, or with no key configured (`LLM_PROVIDER=none`), the event stays UNRECOGNIZED.
